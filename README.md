...
Requires the [GitHub CLI](https://cli.github.com/) (`gh`). Replace `dynamo-ca58362-games-puzzles-and-interactive-simulation`
with the repository you were assigned:

```bash
gh repo fork handshake-project-dynamo/dynamo-ca58362-games-puzzles-and-interactive-simulation --clone
cd dynamo-ca58362-games-puzzles-and-interactive-simulation
git checkout -b submission
#  ... build the task (see below) ...
git add -A && git commit -m "Task submission"
git push -u origin submission
gh pr create --repo handshake-project-dynamo/dynamo-ca58362-games-puzzles-and-interactive-simulation --fill
```
...
- **Validation** — builds your environment and confirms your reference solution
  ...
  Two agent models are used in the trial stages — **Model A** and **Model B**. **Only Model A
  gates your PR.** Model B runs purely for our own calibration and **never blocks** you. The
  comments label every trial result by model so you can tell them apart.
...
- **Timeout pre-check (pass@2)** — runs the agent twice under a hard 3600-second (1 hour) budget on
  ...
  the task is plausibly hard before the full trials. **The Model A pass@2 is the gate** — it
  ...
  in Slack), or a failure looks like a task/verifier bug (→ fix the task or tests). A **Model B
  pass@2 runs at the same time but is non-blocking** — it posts its own comment for reference and
  can never fail your PR.
- **Agent trials (pass@5)** — **Model A** (the same gating model as the pass@2 above — *not*
  Model B) attempts your task **5 times**. Your task must be **hard enough that at least 3 of the
